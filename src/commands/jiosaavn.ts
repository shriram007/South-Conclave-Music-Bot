...
import {
  isJioSaavnUrl,
  loadJioSaavnAsLavalinkTrack,
  resolveJioSaavnTrack,
  resolveJioSaavnUrl,
  searchJioSaavn,
} from "../services/jiosaavn.js";

async function executeJioSaavn(interaction: ChatInputCommandInteraction) {
  ...
  console.log(
    `[JioSaavn Command] User: "${interaction.user.tag}" (${interaction.user.id}) in "${interaction.guild?.name}" | Command: "${cmdName}" | Query: "${rawQuery}"`
  );
  ...
  await interaction.editReply(
    `❌ No studio tracks found on JioSaavn for: \`${rawQuery}\`\n💡 Try checking spelling, selecting from the autocomplete list, or using \`/play\` for universal search.`
  );
  autoDeleteReply(interaction, 10000);
}

async function autocompleteJioSaavn(interaction: AutocompleteInteraction) {
