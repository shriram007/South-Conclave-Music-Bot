...
export function getSourceInfo(
  ...
  uri?: string,
  userData?: any
): { name: string; quality: string; badge: string; color: number; command: string } {
 ...
 */
export function parseTrackTitle(rawTitle: string, rawAuthor: string = ""): ParsedTrackTitle {
  ...
  if (fromMatch) movieOrAlbum = fromMatch[1].trim();
  ...
  if (title.includes("|")) {
    ...
    if (parts.length === 2) {
      if (/^(official\s+)?(lyric(al)?|video|audio)(\s+(song|video))?$/i.test(parts[1])) {
        ...
        if (!rawAuthor || artist) artist = artist || cleanPart(parts[0]);
        else movieOrAlbum = cleanPart(parts[0]);
      }
    } else {
