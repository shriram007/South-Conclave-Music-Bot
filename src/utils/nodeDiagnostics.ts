...
export function nodeErrorSummary(error: unknown): string {
  ...
  const visit = (value: unknown, depth = 0) => {
    ...
    if (typeof e.code === 'string' && /^[A-Z][A-Z0-9_]{1,60}$/.test(e.code)) codes.add(e.code);
    visit(e.cause, depth + 1);
    if (Array.isArray(e.errors)) e.errors.slice(0, 8).forEach(child => visit(child, depth + 1));
  };
  visit(error);
  if (codes.has('ECONNREFUSED')) return `${[...codes].join(', ')}: no server accepted the connection; check Lavalink host, port and container allocation.`;
  if (codes.has('ENOTFOUND') || codes.has('EAI_AGAIN')) return `${[...codes].join(', ')}: Lavalink hostname could not be resolved.`;
  if (codes.size) return `${[...codes].join(', ')}: check node reachability, TLS and the Lavalink server logs.`;
  ...
  if (status) return `WebSocket HTTP ${status[1]}: check Lavalink password, port, TLS and reverse-proxy WebSocket support.`;
