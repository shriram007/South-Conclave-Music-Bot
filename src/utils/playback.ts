...
export function seekToken(player: any): string {
...
export function canSeek(track: any): boolean {
...
export function parseSeek(value: string, position: number, duration: number): number | null {
  const match = value.trim().match(/^([+-]?)(\d+(?::\d{1,2}){0,2})(?:s)?$/i);
  if (!match || !Number.isFinite(duration) || duration <= 1000) return null;
...
export async function withTimeout<T>(work: Promise<T>, ms: number): Promise<T | null> {
  ...
  try { return await Promise.race([work, new Promise<null>(r => { timer = setTimeout(() => r(null), ms); })]); }
...
export class RecentFailures {
  ...
  constructor(private ttlMs = 180000, private now = () => Date.now()) {}
  add(id: string): void {
  ...
  has(id: string): boolean {
    ...
    if (expiry <= this.now()) { this.entries.delete(id); return false; }
    return true;
  }
