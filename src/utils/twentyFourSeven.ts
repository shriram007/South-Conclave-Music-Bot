...
export interface Server247Config {
  enabled: boolean;
  voiceChannelId: string;
  textChannelId?: string;
}

const configCache = new Map<string, Server247Config>();
  ...
  try {
    ...
    for (const [guildId, cfg] of Object.entries(json)) {
      if (cfg && typeof cfg === "object") {
        configCache.set(guildId, cfg as Server247Config);
      }
    }
...
export function get247Config(guildId: string): Server247Config | undefined {
  return configCache.get(guildId);
}

export function set247(guildId: string, enabled: boolean, voiceChannelId?: string, textChannelId?: string): void {
  ...
  try {
    ensureFile();
    const obj: Record<string, Server247Config> = {};
 ...
 */
export async function rejoin247Channels(client: Client): Promise<void> {
  for (const [guildId, cfg] of configCache.entries()) {
    ...
    const channel = guild.channels.cache.get(cfg.voiceChannelId) as VoiceBasedChannel | undefined;
