  ...
  try {
    ...
    if (voiceChannel.bitrate < maxBitrate) {
      ...
      if (botMember?.permissions.has("ManageChannels")) {
        await (voiceChannel as any).setBitrate(maxBitrate, "South Conclave Audiophile Auto-Optimization").catch(() => {});
...
export function isSameSongOrJunk(candidateTitle: string, previousTracks: any[]): boolean {
  ...
  if (junkKeywords.some((j) => lowTitle.includes(j))) return true;
        ...
        if (!recoveredTrack && track.info.uri && fastNodes.length > 0 && !trySoundCloudFirst) {
          for (const node of fastNodes) {
            try {
              ...
              const directTrack = directRes?.tracks?.find(
                (t: any) => !restrictedTrackIds.has(t.info.identifier)
              );
              if (directTrack) {
...
export function getBestNode(): string | undefined {
  ...
  const scored = candidates.map((node) => {
    ...
    if (node.id === "Kasawa-MasterNode") priorityBonus = -500; // Primary master node (direct HTTP 320k JioSaavn + YT/SoundCloud)
