 ...
 */
const originalPlayerPlay = (Player.prototype as any).play;
let playPatched = false;

export function patchPlayerPrototypePlay(): void {
  if (playPatched) return;
  playPatched = true;

  (Player.prototype as any).play = async function (this: Player, options: any = {}) {
    let targetTrack: any = options?.clientTrack || options?.track || this.queue.current || this.queue.tracks[0];
    ...
    if (targetTrack && this.node) {
      ...
      const isSpotify = targetTrack.info?.sourceName === "spotify" || (targetTrack.userData as any)?.sourceName === "spotify";
      const hasSpotifySupport = (this.node as any).info?.sourceManagers?.includes("spotify");
      if (isSpotify && !hasSpotifySupport && this.node.connected && isNodeHealthy(this.node.id)) {
        ...
        try {
          ...
          const searchRes: any = await Promise.race([
          ...
          if (searchRes?.tracks?.length) {
            ...
            (targetTrack.userData as any).nodeId = this.node.id;
            (targetTrack.userData as any).convertedFromSpotify = true;
          }
        } catch (e) {
      ...
      const trackNodeId = (targetTrack.userData as any)?.nodeId;
      if (trackNodeId && trackNodeId !== this.node.id) {
        const nodeManager = (this as any).LavalinkManager?.nodeManager || lavalink?.nodeManager;
        ...
        if (searchUri && this.node.connected && isNodeHealthy(this.node.id)) {
          try {
            ...
            const localRes: any = await Promise.race([
            ...
            if (localRes?.tracks?.length) {
              ...
              if (options?.clientTrack) options.clientTrack.encoded = native.encoded;
              if (options?.track) options.track.encoded = native.encoded;
              (targetTrack.userData as any).nodeId = this.node.id;
              reResolved = true;
            }
          } catch {}
    ...
    try {
      ...
      if (
        errMsg.includes("Failed to decode track") ||
        ...
        if (searchUri && nodeManager?.nodes) {
          const altNodes = Array.from(nodeManager.nodes.values())
            .filter((n: any) => n.connected && n.id !== this.node?.id && isNodeHealthy(n.id));
          for (const altNode of altNodes) {
            try {
              const res: any = await Promise.race([
                (altNode as any).search({ query: searchUri }, currentTrack?.requester),
                new Promise<null>((r) => setTimeout(() => r(null), 3000)),
              ]);
              if (res?.tracks?.length) {
                const freshTrack = res.tracks[0];
                freshTrack.userData = { ...(currentTrack.userData || {}), nodeId: (altNode as any).id };
                freshTrack.requester = currentTrack.requester;
                await this.changeNode(altNode as any, false);
                if (this.queue.current) this.queue.current = freshTrack;
...
async function discoverAutoplayRecommendation(player: Player, seedTrack: Track): Promise<Track | null> {
  ...
  if (player.node?.connected && isNodeHealthy(player.node.id) && (studioMasterTrack as any).userData?.nodeId !== player.node.id) {
    ...
    if (uriToSearch) {
      try {
        const localRes: any = await withTimeout(player.node.search({ query: uriToSearch }, seedTrack.requester), 2500);
        if (localRes?.tracks?.length) {
          const native = localRes.tracks[0];
