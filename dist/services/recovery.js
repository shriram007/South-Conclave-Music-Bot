...
export async function resolveRecoveryTrack(original, candidateNodes, isCurrent) {
    ...
    const search = async (node, query, source) => {
        if (!active())
            return [];
        try {
            const result = await withTimeout(node.search({ query, ...(source ? { source } : {}) }, original.requester), Math.min(3000, deadline - Date.now()));
    ...
    if (!original.userData?.isJioSaavn) {
        const results = await Promise.all(nodes.map(async (node) => {
            ...
            if (track)
                track.userData = { ...track.userData, searchSource: 'ytmsearch' };
