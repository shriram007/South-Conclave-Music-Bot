...
export async function smartSearch(player, query, isUrl, user) {
    ...
    if (isUrl) {
        ...
        const safeNodeSearchUrl = async (targetNode) => {
            let timer;
            try {
                return await Promise.race([
                    targetNode.search({ query }, user),
                    new Promise((r) => { timer = setTimeout(() => r(null), 4000); }),
                ]);
            }
            catch {
                return null;
    ...
    const tryJioSaavnResolution = async (reason) => {
        try {
    ...
    const trySoundCloudResolution = async (reason) => {
    ...
    async execute(interaction) {
        ...
        if (isSpotifyPlaylistOrAlbum(rawQuery)) {
            ...
            if (collection.thumbnail)
                initialEmbed.setThumbnail(collection.thumbnail);
            ...
            if (queuedCount > 0) {
                ...
                if (collection.thumbnail)
                    finalEmbed.setThumbnail(collection.thumbnail);
