    ...
    async execute(interaction) {
        ...
        if (subcommand === "add-song") {
            ...
            if (player) {
                ...
                const dummyPlayer = {
                    node: defaultNode,
                    search: (opts, user) => defaultNode ? defaultNode.search(opts, user) : Promise.resolve(null),
                };
            ...
            if (newTrack.artworkUrl)
                embed.setThumbnail(newTrack.artworkUrl);
        ...
        if (subcommand === "add-queue") {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`Saved **${res.addedCount}** new tracks from the current queue into **${playlistName}**!\nTotal songs in playlist: **${res.totalTracks}**`)
        ...
        if (subcommand === "play") {
            ...
            for (let i = 0; i < playlist.tracks.length; i += BATCH_SIZE) {
                ...
                const resolvedBatch = await Promise.all(batch.map(async (t) => {
                    try {
