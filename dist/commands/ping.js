...
function getPingBadge(ms) {
    if (ms < 0 || isNaN(ms))
        return "`-- ms`";
    if (ms <= 60)
        return `🟢 \`${Math.round(ms)}ms\` *(Ultra Fast)*`;
    if (ms <= 150)
            ...
            embed.addFields([
                {
                    ...
                    value: `• **Node:** \`${activeNode.id}\` (${activeNode.options.host})\n` +
