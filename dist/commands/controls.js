...
export function parseIndicesToRemove(trackInput, toInput, queueLength = 0) {
    ...
    for (const chunk of input.split(/[,;\s]+/).filter(Boolean)) {
        ...
        if (!match)
            return { indices: [], error: 'Use track numbers or ranges, such as 1, 3, 5-8.' };
        ...
        for (let n = Math.max(1, Math.min(from, to)); n <= Math.min(queueLength, Math.max(from, to)); n++)
            indices.add(n);
    }
