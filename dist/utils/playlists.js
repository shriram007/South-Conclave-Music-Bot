    ...
    try {
        ...
        for (const [userId, playlistsObj] of Object.entries(json)) {
            ...
            if (typeof playlistsObj === "object" && playlistsObj !== null) {
                for (const [nameKey, p] of Object.entries(playlistsObj)) {
 ...
 */
export function addSongToPlaylist(userId, name, track) {
 ...
 */
export function addQueueToPlaylist(userId, name, tracks) {
    const cleanKey = name.trim().toLowerCase();
    let userMap = playlistsCache.get(userId);
    if (!userMap) {
        userMap = new Map();
