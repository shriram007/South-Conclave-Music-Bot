 ...
 */
export function saveActiveSessions() {
    if (!lavalink || !lavalink.players)
        return;
    try {
        ensureDirectory();
        const sessions = {};
 ...
 */
function cleanTrackForSerialization(track) {
    return {
 ...
 */
export function clearGuildSession(guildId) {
    try {
        ...
        for (const session of sessionList) {
            try {
                ...
                if (session.repeatMode) {
                    await player.setRepeatMode(session.repeatMode).catch(() => { });
                }
                if (!player.connected) {
                ...
                if (session.currentTrack) {
                    ...
                    console.log(`[Session Recovery] Resumed "${session.currentTrack.info?.title}" in "${guild.name}" at ${formatDuration(seekPos)}`);
