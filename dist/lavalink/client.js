...
export function recordYouTubePlaybackFailure() {
    ...
    while (ytPlaybackFailures.length > 0 && ytPlaybackFailures[0] < cutoff)
        ytPlaybackFailures.shift();
}
export function recordYouTubePlaybackSuccess() {
    // A verified success resets the health tracker
    ytPlaybackFailures.length = 0;
}
export function isYouTubePlaybackHealthy() {
    const cutoff = Date.now() - YT_HEALTH_WINDOW_MS;
    while (ytPlaybackFailures.length > 0 && ytPlaybackFailures[0] < cutoff)
        ytPlaybackFailures.shift();
...
async function discoverAutoplayRecommendation(player, seedTrack) {
        ...
        for (const node of nodesToTry) {
            try {
                ...
                if (radioRes?.tracks?.length && radioRes.loadType !== "error" && radioRes.loadType !== "empty") {
                    const validCandidates = radioRes.tracks.filter((t) => isPreferredRadioUpload(t.info) &&
    ...
    if (!foundCandidate && ytHealthy) {
        ...
        if (seedLang !== "global" && seedLang !== "english") {
            queriesToTry.push(`${fullSeedQuery} songs`, `${cleanTitle} similar ${seedLang} songs`, `${effectiveArtist} ${seedLang} hit songs`, `${effectiveArtist} ${seedLang} radio`);
        }
        else {
            queriesToTry.push(`${fullSeedQuery} similar songs`, `${cleanTitle} mix`, `${effectiveArtist} similar artists`, `${effectiveArtist} top tracks`);
        }
            ...
            for (const node of nodesToTry) {
                try {
                    ...
                    if (recRes?.tracks?.length && recRes.loadType !== "empty" && recRes.loadType !== "error") {
                        const candidate = [...recRes.tracks].sort((a, b) => authorConfidence(b.info.author) - authorConfidence(a.info.author)).find((t) => isPreferredRadioUpload(t.info) &&
                            !historyIds.has(t.info.identifier) &&
                            !restrictedTrackIds.has(t.info.identifier) &&
                            !isSameSongOrJunk(t.info.title, [...player.queue.previous, ...(player.queue.current ? [player.queue.current] : [])]) &&
                            (t.info.duration || 0) >= 60000 &&
    ...
    if (!foundCandidate) {
        try {
            ...
            for (const scQuery of scQueries) {
                ...
                for (const node of nodesToTry) {
                    try {
                        const scRes = await withTimeout(node.search({ query: scQuery, source: "scsearch" }, seedTrack.requester), 3500);
                        if (scRes?.tracks?.length && scRes.loadType !== "empty" && scRes.loadType !== "error") {
                            const candidate = scRes.tracks.find((t) => !historyIds.has(t.info.identifier) &&
                                !restrictedTrackIds.has(t.info.identifier) &&
                                !isSameSongOrJunk(t.info.title, [...player.queue.previous, ...(player.queue.current ? [player.queue.current] : [])]) &&
                                (t.info.duration || 0) >= 60000 &&
                                (t.info.duration || 0) <= 900000);
...
export function initLavalink(client) {
    ...
    if (!getMasterNodeConfigs().length)
        throw new Error('No Lavalink nodes configured: enable a custom node or public fallbacks.');
