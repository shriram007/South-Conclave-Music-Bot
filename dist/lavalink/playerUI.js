...
export function buildPlayerMessage(player, track) {
    ...
    if (!current)
        return { embeds: [new EmbedBuilder().setColor(0x5865f2).setDescription('The queue is empty. Use `/play` or `/jio` to add music.')], components: [] };
    ...
    const preset = player.getData('filter_preset_key') || 'reset';
    const requester = current.requester;
    const requesterId = requester?.id || current.userData?.userId;
    const isRadio = Boolean(current.userData?.isAutoplay);
    ...
    const queueCount = player.queue.tracks.filter(t => !t.userData?.isAutoplay).length;
    const radioNext = player.queue.tracks.some(t => t.userData?.isAutoplay);
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: 'Up next', value: `${queueCount} queued${radioNext ? ' · Radio ready' : ''}`, inline: true }, { name: 'Volume', value: `${player.volume}%`, inline: true }, { name: 'Repeat', value: player.repeatMode === 'track' ? 'This track' : player.repeatMode === 'queue' ? 'Queue' : 'Off', inline: true })
        .setFooter({ text: `${source.name} · ${player.getData('eq_preset') || 'Flat'}${player.getData('normalized') ? ' · Normalized' : ''}` });
    if (/^https?:\/\//i.test(current.info.uri || ''))
        embed.setURL(current.info.uri);
    if (/^https?:\/\//i.test(current.info.artworkUrl || ''))
        embed.setThumbnail(current.info.artworkUrl);
    const button = (id, label, disabled = false, style = ButtonStyle.Secondary) => new ButtonBuilder().setCustomId(id).setLabel(label).setStyle(style).setDisabled(disabled);
    const transport = new ActionRowBuilder().addComponents(button('player_prev', 'Previous', !player.queue.previous.length), button('player_pause_resume', player.paused ? 'Resume' : 'Pause', false, ButtonStyle.Primary), button('player_skip', 'Next'), button('player_stop', 'Stop'), button('player_like', liked ? '♥ Saved' : '♡ Save', isRadio));
    const controls = new ActionRowBuilder().addComponents(button(`player_seek_step:${token}:back`, '−15s', !seekable), button(`player_seek:${token}`, 'Seek…', !seekable), button(`player_seek_step:${token}:forward`, '+15s', !seekable), button('player_autoplay', autoplay ? 'Radio: on' : 'Radio: off', false, autoplay ? ButtonStyle.Success : ButtonStyle.Secondary), button('player_queue', `Queue (${queueCount})`));
    const components = [transport, controls];
    if (seekable) {
        ...
        components.push(new ActionRowBuilder().addComponents(timeline));
    }
    const presets = [
    ...
    components.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('player_filter_menu').setPlaceholder('Sound settings').addOptions(presets.map(p => ({ ...p, default: p.value === preset })))));
