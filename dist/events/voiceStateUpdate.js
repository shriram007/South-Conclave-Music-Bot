...
export function handleVoiceStateUpdate(oldState, newState, client) {
    ...
    if (humanMembers.size === 0) {
        ...
        if (emptyChannelTimers.has(guild.id))
            return;
        console.log(`[VoiceState] Everyone left voice channel "${botVoiceChannel.name}" in "${guild.name}". Disconnecting in 20s if no one rejoins...`);
        const timer = setTimeout(async () => {
            ...
            if (!currentHumans || currentHumans.size === 0) {
                ...
                if (activePlayer.textChannelId) {
                    const textChannel = guild.channels.cache.get(activePlayer.textChannelId);
                    if (textChannel) {
                        textChannel
                            .send(`👋 Left **${botVoiceChannel.name}** because everyone left the voice channel.\n*Equalizer reset to **Normal (Flat)**.*`)
                            .catch(() => { });
                    }
                }
        ...
        if (emptyChannelTimers.has(guild.id)) {
            console.log(`[VoiceState] Member active in voice channel "${botVoiceChannel.name}". Cancelled empty channel disconnect timer.`);
            clearTimeout(emptyChannelTimers.get(guild.id));
            emptyChannelTimers.delete(guild.id);
        }
    }
